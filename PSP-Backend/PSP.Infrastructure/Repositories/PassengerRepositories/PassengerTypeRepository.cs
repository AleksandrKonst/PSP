...
{
    public async Task<List<PassengerType>> GetAllAsync() => await context.DictPassengerTypes.ToListAsync();
    
    public async Task<List<PassengerType>> GetPartAsync(int index, int count) => await context.DictPassengerTypes.Skip(index).Take(count).ToListAsync();

    public async Task<PassengerType?> GetByIdAsync(string code) => await context.DictPassengerTypes.Where(p => p.Code == code).FirstOrDefaultAsync();

    public async Task<int> GetCountAsync() => await context.DictPassengerTypes.CountAsync();
