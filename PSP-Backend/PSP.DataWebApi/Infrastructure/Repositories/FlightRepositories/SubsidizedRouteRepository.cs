...
public class SubsidizedRouteRepository(PSPContext context) : ISubsidizedRouteRepository
{
    ...
    public async Task<IEnumerable<SubsidizedRoute>> GetAllAsync() => await context.SubsidizedRoutes.ToListAsync();

    public async Task<IEnumerable<SubsidizedRoute>> GetPartAsync(int index = 0, int count = Int32.MaxValue) => await context.SubsidizedRoutes.Skip(index).Take(count).ToListAsync();

    public async Task<SubsidizedRoute?> GetByCodeAsync(long code) => await context.SubsidizedRoutes.Where(p => p.Id == code).FirstOrDefaultAsync();

    public async Task<long> GetCountAsync() => await context.SubsidizedRoutes.CountAsync();

    public async Task<bool> CheckByCodeAsync(long code) => await context.SubsidizedRoutes.Where(p => p.Id == code).AnyAsync();
